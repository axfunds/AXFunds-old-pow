...
//tcgen
uint256 hashGenesisBlock("0xd1d5329c28dfd4ee8907ca15584f8245badb7438c4004f5f977143a4fb1a61b9");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AXFunds: starting difficulty is 1 / 2^12
...
{
    // AXFunds: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
	// - diffblock coin implmented base on difficulty 
	if (nHeight==1)
		return 78120000*COIN;
...
static const int64 nTargetTimespan = 4 * 60 * 60; // axfunds: every 4 hours
static const int64 nTargetSpacing = 60; // axfunds: 1 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AXFunds: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if(pindex->pprev)
    {
		...
		prevBits = pindex->pprev->nBits;
		if ( pindex->pprev->pprev)
			diffTime = pindex->pprev->GetBlockTime()- pindex->pprev->pprev->GetBlockTime();
    }

	
	int64 blockValue = GetBlockValue(diffTime,prevBits, pindex->nHeight, nFees, prevHash);
    if (vtx[0].GetValueOut() > blockValue )
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(),blockValue));
...
int GetOurChainID()
{
    return 0x0058; // AXFunds Chain ID for merge mining
}
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AXFunds: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // AXFunds: temporarily disable v2 block lockin until we are ready for v2 transition
    // return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb44cc80bfa2a5629c618d5a3c07f2400462d781c1a289379576dbb4fc29618c3");
    }
...
bool InitBlockIndex() {
	...
	//tcgen
        block.nTime    = 1417912409;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 21100558;
...
*///end of genius block
		...
		//tcgen
	 assert(block.hashMerkleRoot == uint256("0x5d76f86103766b4e8acd1e8348e2ab6848007249d2efed4a5de48a694a7fb247"));
...
unsigned char pchMessageStart[4] = { 0x19, 0x43, 0x19, 0x43 }; // AXFunds: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
		...
		int diffTime = 0;
		if ( pindexPrev->pprev)
			diffTime = pindexPrev->GetBlockTime()- pindexPrev->pprev->GetBlockTime();

        pblock->vtx[0].vout[0].nValue = GetBlockValue(diffTime,pindexPrev->nBits, pindexPrev->nHeight+1, nFees, pindexPrev->GetBlockHash());
...
//    printf("proof-of-work found  \n  hash: %s  \ntarget: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("axfundsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("axfundsMiner : ProcessBlock, block not accepted");
    }
...
void static axfundsMiner(CWallet *pwallet)
{
    printf("axfundsMiner started\n");
    ...
    try { loop {
        ...
        printf("Running axfundsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("axfundsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&axfundsMiner, pwallet));
}
